...
/**
 * Recordatorios escalonados de pagos a proveedores (decisión Mauricio
 * 2026-07-20): campanita a gerencia/recepción a los 7 días, 3 días y
 * el DÍA del vencimiento de cada cuenta por pagar con saldo, más un
 * aviso único cuando la cuenta ya venció impaga. Espejo exacto de la
 * cobranza, pero de lo que DEBEMOS.
 *
 * Lo dispara el scheduler una vez al día (pagos:avisar-vencimientos).
 ...
 * Cuentas pagadas (saldo 0) salen solas del radar por el scope. Cambiar
 * la fecha de vencimiento reinicia el escalón (NULL) y rearma el ciclo.
 */
final readonly class AvisarVencimientosPagosService
{
    /** Marca de "ya avisé que venció". */
    private const int VENCIDA = -1;

    public function __construct(private NotificadorPagos $notificador) {}
    ...
    public function avisar(): int
    {
        ...
        $cuentas = CuentaPorPagar::query()
            ->with('proveedor:id,nombre', 'compra:id,codigo')
            ->where('saldo', '>', 0)
     ...
     */
    private function escalonDeHoy(CuentaPorPagar $cuenta, Carbon $hoy): ?int
    {
     ...
     */
    private function tocaAvisar(CuentaPorPagar $cuenta, int $escalon): bool
    {
